...
export default function New_post_icon_Item({ newsItem, classItem_all }: newsItemStory) {
  return (
    <Tippy className='bg-[#F6F6F6]' arrow={false} content={<span className={'text-black'}>{newsItem.story}</span>}>
      <div className='flex py-2 cursor-pointer item-center'>
        <div
          className={classNames(
            `w-[7%] border-r-[0.5px] border-stale-300 text-[#62748D] hover:text-[#191970] font-normal text-center	${
        ...
        <div
          className={classNames(
            `line-clamp-2 text-[#182940] text-[15px] hover:text-[#191970] font-medium	w-[90%] ml-3 ${
              classItem_all ? 'text-[10px]' : 'text-[14px]'
            }`
          )}
        >
